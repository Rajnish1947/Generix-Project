    ...
    <>
      ...
      >
        ...
        <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8 py-3 sm:py-4 flex items-center justify-between">
          ...
          {/* Desktop Links */}
          <ul
            className={`hidden md:flex items-center gap-6 lg:gap-8 text-sm font-medium transition
              ${
                scrolled ? (dark ? "text-white" : "text-black") : "text-white"
              }`}
          >
      ...
      {open && (
        <div
          className={`fixed md:hidden top-[60px] inset-x-0 bottom-0 z-40 backdrop-blur-xl
          ${dark ? "bg-black/95 text-white" : "bg-white/95 text-black"}`}
        >
          <div className="flex flex-col justify-between h-full px-5 sm:px-6 py-6 overflow-y-auto">
            {/* Links */}
            <ul className="space-y-4 mt-4 text-base font-medium">
              {links.map((item) => (
                <li
                  key={item.label}
                  className={`pb-4 ${
