...
export default function Navbar() {
  ...
  // ✅ ALL ROUTES LOWERCASE (Pages Router rule)
  const links = [
    { label: "Resources", href: "/features" },
    { label: "Documentation", href: "/documentation" },
    { label: "Customers", href: "/startups" },
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-6xl px-10 py-4 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-2">
          <img
            src="/mintlify-wordmark.svg"
            alt="Mintlify"
            className={`h-6 w-auto transition ${scrolled ? "invert" : ""}`}
          />
        </Link>
      ...
      {/* Mobile Menu */}
      {open && (
        <div className="fixed md:hidden top-16 inset-x-0 bottom-0 bg-white">
          <ul className="p-6 space-y-4 text-black">
            {links.map((item) => (
              <li key={item.label}>
